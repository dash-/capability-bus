...
In a client‑side application — a single‑page app running in a browser — the AI agent cannot call the application. The application can make outbound requests to an LLM API, but nothing can call back into it. The only way an AI agent can act in the application is through the response path: structured tool‑use objects that come back in the LLM's reply, which the application then executes locally. The application is the orchestrator, not the AI.

This topological constraint is the starting point for this paper. Existing patterns for AI tool integration — such as Anthropic's Model Context Protocol — assume the application can expose a server that the AI connects to. When the application is a client‑side SPA, that assumption does not hold. This paper addresses the gap: how does an application that *cannot be called* give an AI agent the ability to act?

The answer is a **Capability Bus** that mediates all meaningful application actions, and an **Agent Interface** that exposes those actions to the AI through a structured, permissioned protocol. The bus is the application's own internal dispatch layer — buttons, keyboard shortcuts, and gestures all invoke capabilities through it. The agent becomes one more caller into the same surface, subject to the same validation, permissions, and audit logging. From the bus's perspective, a button click and an agent invocation are the same thing.

Communication between the agent and the application happens through a **Structured Agent Invocation Protocol** (hereafter "the invocation protocol"): a deterministic, typed contract for requesting and receiving the results of application actions. Agent commands are structured objects with explicit capability names, validated arguments, request IDs, and idempotency keys — not natural language embedded in chat text or escape sequences parsed from markdown.

The architecture synthesizes established patterns — command buses, centralized dispatch, tool‑calling LLM interfaces — but the specific composition around the client‑side topology constraint, and the protocol designed for it, have not been formalized before. The result is a system where a user can say "submit my order" into a chat panel, and the application performs exactly the same operation that would occur if the user had clicked the Submit button. No selectors. No screen scraping. No external service calls. The application remains the sole authority on what it can do and who is allowed to do it.
...
When a product team wants to add an AI chat panel to a client‑side application — a React SPA, a Vue app, a mobile webview — they face a fundamental constraint that is easy to overlook. The application runs in the user's browser. It can make outbound requests to a web service, and through that service to an LLM API, but the application itself is not addressable. The AI cannot initiate a connection to it. The only channel the AI has to act in the application is the response path: tool‑use objects embedded in the LLM's reply, which travel back through the web service to the client, where the application must interpret and execute them locally.

This is a different topology from the one that existing AI tool integration patterns assume. MCP, external tool servers, and function‑calling APIs all presume that the tool provider is a server the AI can reach. In a client‑side application, there is no server to reach — at least not one that has access to the application's UI state, local data, and in‑page business logic. The application must define an internal action surface and make it available to the AI through the only channel that exists: the response path.

This constraint alone is solvable — an application could simply parse tool calls from the LLM response and wire them to handlers. But doing this naively runs into three problems that make ad hoc approaches fragile, insecure, and unmaintainable.

### Action Fragmentation

Business logic in a typical single‑page application is scattered across click handlers, keyboard shortcuts, effects, form submissions, and implicit UI flows. A React application might implement "add item to cart" as an `onClick` callback in one component, a keyboard shortcut handler in another, and a drag‑and‑drop handler in a third—each with subtly different validation logic, error handling, and side effects. There is no single authoritative representation of "what this application can do." If the AI's tool calls are wired to individual handlers ad hoc, the agent inherits all of this fragmentation — different code paths, different validation, different behavior depending on which handler it happens to call.

### AI as Narrator, Not Actor

Most AI assistants embedded in applications are limited to describing UI features rather than invoking them. An AI copilot in a project management tool might say "click the gear icon in the top right, then select 'Integrations' from the dropdown," forcing the user to manually translate intent into gestures. This is especially harmful for accessibility: users who need AI assistance the most are precisely those for whom "click the third button" is the hardest instruction to follow. The AI knows what the user wants. The application knows how to do it. But without a structured action surface, there is no bridge between them.
...
These problems share a common root: the absence of a stable, semantic, application‑level action surface. The application has no unified concept of "the things I can do" that is independent of how those things are presented in the UI. Introducing this abstraction solves the topology problem — giving the AI a structured surface to act through — while simultaneously resolving the fragmentation, narration, and brittleness problems that existed before AI entered the picture.
...
Anthropic's Model Context Protocol provides a standardized way for AI agents to discover and invoke tools exposed by external servers. MCP solves the topology where the tool provider is a server that the AI can connect to over a transport layer — a database, a SaaS API, a local file system exposed through a server process.

This paper addresses a different topology: one where the application is a client that cannot be called. In a browser‑based SPA, there is no server endpoint for the AI to connect to, and the application's UI state, local data, and in‑page business logic are not accessible from outside the browser. The Capability Bus is not an external service built for the agent; it is the application's own internal dispatch layer, and the agent acts through the response path — tool‑use objects in the LLM's reply that the application executes locally against its own bus.

The distinction is architectural, not competitive. MCP is "build an external tool server for the agent." The Capability Bus is "refactor your application's internal architecture so that the agent gets access as a side effect." The latter is a much easier sell to teams that are not yet committed to AI integration, because the bus improves the application's architecture — thinner components, unified audit logging, testable business logic — regardless of whether an agent is ever connected.

The two approaches are complementary. An application with a well‑defined Capability Bus could also expose its capabilities as an MCP server for external agents, using the same capability definitions. And a full‑stack application might use MCP for server‑side tool integration while using the Capability Bus for client‑side actions, composing both manifests into a unified surface for the agent.
...
None of these components are new in isolation. What this paper contributes is the identification of the client‑side topology problem as a distinct architectural challenge, and a formalization of how established patterns compose to solve it: natural‑language capability descriptions, structured input/output schemas compatible with function calling, side‑effect classification for safety policies, streaming event feedback for conversational context, and a unified manifest format that bridges the application's internal architecture with the agent's tool‑use interface. The contribution is a design discipline and reference architecture, not a new primitive.
...
**Client/server bus composition.** Some capabilities naturally execute on the client (UI state changes, navigation), while others require server‑side execution (payment processing, database writes). A composed architecture would maintain a bus on each side, with the server merging its own capabilities with the client's manifest — received during an initial handshake — into a unified surface for the agent. When the LLM issues a tool call, the server checks whether the capability is local or client‑side: local capabilities execute immediately, while client capabilities are forwarded to the client for execution against its own bus, with the result relayed back to continue the LLM conversation. The agent sees one manifest and issues tool calls without knowing or caring where execution happens. The manifest schema would need an `execution_context` field on each capability entry to support routing, and the composed manifest would need to handle eventually‑consistent availability for server capabilities whose preconditions the client cannot check in real time.

**Manifest‑driven dynamic interfaces.** Because the capability manifest describes what actions are available, contextually relevant, and currently invocable, it can also drive the UI itself. Rather than static menus and fixed button layouts, an application could dynamically surface the most relevant next actions based on the user's current context and recent activity—finishing a cart addition surfaces "Checkout" and "Save for Later," submitting an order surfaces "Track Order" and "Continue Shopping." The simplest version ranks available capabilities client‑side using the manifest directly, with no AI round‑trip. The agent could layer on top for smarter suggestions when it has conversational context the heuristic does not. A further extension: the agent could present pre‑packaged actions as interactive elements directly in the chat—buttons already wired to specific capability invocations with resolved arguments, which the user confirms with a single tap. This would support batched invocation options and presentation modes as natural extensions of the invocation protocol.
